  ...
  render(run: PipelineRun | null): void {
    ...
    this.container.innerHTML = `
      <div class="bg-bg-panel backdrop-blur-xl border border-border-panel rounded-2xl overflow-hidden shadow-2xl shadow-black/40 animate-fade-in">
        <div class="p-3 border-b border-border-panel">
          <span class="text-[10px] font-bold text-txt-muted uppercase tracking-wider">Data Provenance</span>
        </div>
        <div class="p-3 space-y-1.5">
          ${rows.map(([k, v]) => `
            <div class="flex items-center justify-between gap-3 py-1">
              <span class="text-[10px] text-txt-muted uppercase tracking-wider">${k}</span>
              <span class="text-xs text-txt-primary/80 font-mono text-right truncate max-w-[65%]" title="${v}">${v}</span>
            </div>
          `).join('')}
        </div>
      </div>
